...
/**
 * 腾讯云短信配置实现类 腾讯云发送短信时，需要额外的参数 sdkAppId,
 *
 */
@Data
public class TencentSmsChannelProperties extends SmsChannelProperties {
	...
	/**
	 * 考虑到不破坏原有的 apiKey + apiSecret 的结构， 所以腾讯云短信存储时，将 secretId 拼接到 apiKey 字段中，格式为
	 * "secretId sdkAppId"。 因此在使用时，需要将 secretId 和 sdkAppId 解析出来，分别存储到对应字段中。
	 */
	public static TencentSmsChannelProperties build(SmsChannelProperties properties) {
		...
		result.setSdkAppId(keys[1]);
		result.setApiKey(keys[0]);
		return result;
	}
