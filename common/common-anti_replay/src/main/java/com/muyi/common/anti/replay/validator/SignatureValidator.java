package com.muyi.common.anti.replay.validator;

import com.alibaba.fastjson.JSON;
import com.muyi.common.anti.replay.props.ReplayProperties;
import com.muyi.common.anti.replay.util.MdFiveUtils;
	...
	public static class SignatureWorker {
		...
		public SignatureWorker data(HttpServletRequest request) {
			...
			return this.nonce(request.getHeader(headerKey.getNonce()))
					...
					.url(request.getHeader(headerKey.getUrl())).token(request.getHeader(headerKey.getToken()))
					.params(request.getParameterMap()).signature(request.getHeader(headerKey.getSignature()))
					.salt(signatureAlgorithm.getSalt());
		}

		public void execute() {
			String digest = MdFiveUtils.digest(this.salt, this.nonce, this.url, this.timestamp, this.token, this.params,
					this.arguments);
